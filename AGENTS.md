...
- `Tooltip` non-fixed mode is pure CSS (hover/focus/pin) with alignment recomputed on interaction. Do not add window scroll/resize listeners for non-fixed tooltips — the dense grids instantiate thousands of them. Fixed mode (type chart) does track the window.
