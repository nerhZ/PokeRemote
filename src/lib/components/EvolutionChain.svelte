<script lang="ts">
    ...
    function formatKey(s: string) { return s.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()); }

    function describe(s: EvolutionStage): string {
        const parts: string[] = [];
        if (s.min_level != null && s.min_level > 0) parts.push(`Level ${s.min_level}`);
        if (s.item) parts.push(`Use ${formatKey(s.item)}`);
        if (s.min_happiness != null && s.min_happiness > 0) parts.push(`Happiness ${s.min_happiness}+`);
        if (s.time_of_day) parts.push(`During ${s.time_of_day}`);
        if (s.held_item && s.held_item !== s.item) parts.push(`Hold ${formatKey(s.held_item)}`);
        ...
        if (s.gender != null) parts.push(s.gender === 1 ? "Female only" : "Male only");
        if (s.known_move_type) parts.push(`${formatKey(s.known_move_type)}-type move`);
...
{#if stage}
    <div class="flex flex-col items-center gap-2">
        <a href={resolve(`/pokemon/${stage.name}`)} class="flex flex-col items-center gap-1.5 no-underline group">
            ...
            <span class="text-xs font-semibold" style={stage.name === currentName ? `color: ${color}` : "color: rgba(255,255,255,0.5)"}>{formatName(stage.name)}</span>
        </a>
                ...
                {#each stage.children as child}
                    <div class="flex flex-col items-center gap-1">
                        <div class="flex flex-col items-center px-1">
                            ...
                            <span class="text-[10px] text-white/40 text-center max-w-18 leading-tight">{describe(child)}</span>
                        </div>
