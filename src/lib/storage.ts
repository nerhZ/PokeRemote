...
export type FavEntry = { id: number; name: string; image: string; types: string[] };
...
export function toggleFavorite(entry: FavEntry): FavEntry[] {
    ...
    const next = idx >= 0 ? list.filter((f) => f.id !== entry.id) : [entry, ...list].slice(0, 48);
    write(FAV_KEY, next);
    return next;
}
...
export function getSavedTeams(): { name: string; ids: number[]; names: string[] }[] {
    return read(TEAM_KEY, []);
}

export function saveTeam(name: string, members: { id: number; name: string }[]) {
    const teams = getSavedTeams().filter((t) => t.name !== name);
    teams.unshift({ name, ids: members.map((m) => m.id), names: members.map((m) => m.name) });
