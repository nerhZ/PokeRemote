...
export function useAutocomplete() {
    ...
    function search(query: string, exclude?: number[]): { name: string; id: number }[] {
        const excludeSet = new Set(exclude ?? []);
        const q = query.trim().toLowerCase();
        const base = q ? allNames.filter((n) => n.name.toLowerCase().includes(q) || String(n.id).includes(q)) : allNames;
    ...
    return {
        ...
        async loadDetail(name: string): Promise<PokemonDetail | null> {
            try { return await getPokemonDetail(name); } catch { return null; }
        }
    };
}
