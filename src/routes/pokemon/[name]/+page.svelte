...
<div class="min-h-[calc(100vh-73px)] relative" style="background: linear-gradient(180deg, {primaryColor}18 0%, transparent 55%)">
    <div class="max-w-6xl mx-auto px-4 md:px-6 py-6 relative">
        <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
            <a href={resolve('/')} class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-sm font-semibold text-white/60 hover:text-white transition-all no-underline">← Pokédex</a>
            {#if pokemon}
                <div class="flex items-center gap-2">
                    {#if prevId != null}
                        <a href={resolve(`/pokemon/${prevId}`)} class="w-9 h-9 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-white/50 hover:text-white no-underline" aria-label="Previous species">‹</a>
                    {/if}
                    {#if nextId != null}
                        <a href={resolve(`/pokemon/${nextId}`)} class="w-9 h-9 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-white/50 hover:text-white no-underline" aria-label="Next species">›</a>
                    {/if}
                </div>
            {/if}
        </div>
        ...
        {:else if error && !pokemon}
            <EmptyState title="Something went wrong" subtitle={error} actionLabel="Back to Pokédex" onaction={() => goto(resolve('/'))} />
        {:else if pokemon}
            {#if navigating}
